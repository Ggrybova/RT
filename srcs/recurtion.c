...
/*   Updated: 2018/01/20 21:00:59 by vsydorch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	recurtion(t_arr *gl, t_current *src, t_current *cur, int i)
{
	...
	if (cur->mirror && gl->iter[cur->id] < DEPTH)
	{
		...
		if (gl->iter[cur->id] != 0)
			getcol(gl, gl->cur[cur->id][gl->iter[cur->id]]->col,
				gl->cur[cur->id][gl->iter[cur->id]]);
	}
...
{
	create_v(cur[i]->ray->org, cur[i - 1]->p.x,
		cur[i - 1]->p.y, cur[i - 1]->p.z);
	write_mul(cur[i]->ray->dir, &(cur[i - 1]->n), 2 *
		dot(cur[i - 1]->ray->dir, &(cur[i - 1]->n)));
