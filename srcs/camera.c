...
/*   Updated: 2018/01/20 12:46:56 by vsydorch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	multiple_continue(t_arr *gl, t_cam cam, int i)
{
	i = -1;
	while (++i < THREADS)
	{
		gl->cur[i][0]->ray->org->x = ((cam.res[0][0] * gl->eye.x) +
			(cam.res[0][1] * gl->eye.y) + (cam.res[0][2] * gl->eye.z) +
			cam.res[0][3]);
		gl->cur[i][0]->ray->org->y = ((cam.res[1][0] * gl->eye.x) +
			(cam.res[1][1] * gl->eye.y) + (cam.res[1][2] * gl->eye.z) +
			cam.res[1][3]);
		gl->cur[i][0]->ray->org->z = ((cam.res[2][0] * gl->eye.x) +
			(cam.res[2][1] * gl->eye.y) + (cam.res[2][2] * gl->eye.z) +
			cam.res[2][3]);
	}
}
