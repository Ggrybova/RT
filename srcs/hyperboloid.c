...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/01/20 07:34:05 by vsydorch          #+#    #+#             */
/*   Updated: 2018/01/20 08:20:36 by vsydorch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			parab_inter(t_arr *gl, t_ray *ray, t_obj *hprb, int thread)
{
	...
	a[1] = 2 * (dot(ray->dir, &oc) - dot(ray->dir, hprb->v) *
		(dot(hprb->v, &oc) + 2 * hprb->r2));
	a[2] = dot(&oc, &oc) - dot(hprb->v, &oc) * (dot(hprb->v, &oc) +
		4 * hprb->r2);
